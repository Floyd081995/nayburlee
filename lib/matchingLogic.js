...
export async function findMatchingListings(userInput) {
  try {
    ...
    console.log("Listings before duration filter:", matchingListings.map(l => ({id: l.id, duration: l.duration, type: l.type, location_keywords: l.location_keywords, capacity: l.capacity, isActive: l.isActive})));
    ...
    matchingListings = matchingListings.filter(listing => {
      const durations = Array.isArray(listing.duration)
        ...
        : typeof listing.duration === "string" && listing.duration
          ...
          : [];
      const match = durations.some(d => userDurations.includes(d));
      console.log(
        `Listing ${listing.id} durations:`, durations,
